  ...
  try {
    for (let index = 0; index < plannedUpdates.length; index += 1) {
      ...
      try {
        ...
        console.log(
          `[accordion-repair] Applied ${index + 1}/${plannedUpdates.length} ${entry.locale}/${entry.slug} (${entry.itemCount} items)`
        );
      } catch (error) {
        ...
        console.error(
          `[accordion-repair] Failed ${entry.locale}/${entry.slug}: ${message}`
        );
      }
...
main().catch(async (error) => {
  ...
  console.error(`[accordion-repair] Fatal: ${message}`);
  process.exitCode = 1;
});
