...
3.  Ensure the exported JSON files (`data/source/modx/published_resources_flat.json`, `tv_definitions.json`) and the `uploads` folder are accessible to the script.
...
Save the mapping of the old local path to the new Strapi ID and URL. Store this in memory or write it to a `data/source/checkpoints/asset_map.json` file.
...
For every resource in `data/source/modx/published_resources_flat.json`, process the `content`, `introtext`, and any rich text Template Variables (TVs).
...
2.  Check if the path starts with `uploads/` and exists in your `data/source/checkpoints/asset_map.json`.
