...
export async function seedNavigationPermissions(strapi: Core.Strapi): Promise<void> {
  ...
  if (!roleService?.addPermissions || !permissionService?.findMany) {
    strapi.log.warn(
      "[hierarchy-ui] admin role/permission service not available, skipping navigation permission seed",
    );
    return;
  }
  ...
  const registeredActions: string[] | undefined = actionProvider?.values
    ?.()
    ?.map?.((a: { actionId: string }) => a.actionId);
  if (
    !registeredActions ||
    !registeredActions.some((id) => id.startsWith("plugin::navigation."))
  ) {
    strapi.log.warn(
      "[hierarchy-ui] plugin::navigation actions not registered yet, skipping permission seed",
    );
    return;
  }
  ...
  for (const { roleCode, actions } of SEEDS) {
    try {
      ...
      if (toAdd.length === 0) {
        strapi.log.info(
          `[hierarchy-ui] navigation permissions already assigned to ${roleCode}`,
        );
        continue;
      }
      ...
      strapi.log.error(
        `[hierarchy-ui] failed to grant navigation permissions to ${roleCode}`,
        err,
      );
    }
  }
