...
def cmd_prune(target_name: str, *, dry_run: bool, force: bool) -> int:
    if target_name == "production" and not force:
        print(
            "ERROR: Pruning production search_query_log requires --force.\n"
    ...
    print(f"{verb} search_query_log rows on {target_name}...")
    ...
    if dry_run:
        ...
        # DELETE returns no rows in -At mode — success is exit code 0
        print(f"{label}Done.")
        if output:
