#!/usr/bin/env python3
"""Add anonymous search query logging disclosure to privacy policy pages.
...
Interface:
  python3 tools/add_privacy_search_disclosure.py --target=dev
  python3 tools/add_privacy_search_disclosure.py --target=production --force
"""
...
DISCLOSURE_EL = """
<h2>Ανώνυμες αναζητήσεις</h2>
<p>Όταν χρησιμοποιείτε τη λειτουργία αναζήτησης του ιστοτόπου μας, καταγράφουμε ανώνυμα τον όρο αναζήτησης, τη γλώσσα (Ελληνικά ή Ρωσικά) και τον αριθμό των αποτελεσμάτων. Η καταγραφή γίνεται με ένα τυχαίο αναγνωριστικό περιόδου (session UUID) που διαγράφεται αυτόματα όταν κλείνετε το πρόγραμμα περιήγησης.</p>
...
<p>Σκοπός της καταγραφής είναι η βελτίωση των αποτελεσμάτων αναζήτησης και του περιεχομένου μας. Τα δεδομένα διαγράφονται αυτόματα μετά από 90 ημέρες. Δεν κοινοποιούνται σε τρίτους, δεν χρησιμοποιούνται για διαφήμιση και δεν συσχετίζονται με εσάς προσωπικά.</p>
""".strip()

DISCLOSURE_RU = """
<h2>Анонимные поисковые запросы</h2>
<p>При использовании поиска на нашем сайте мы анонимно регистрируем поисковый запрос, язык (греческий или русский) и количество результатов. Регистрация ведется с использованием случайного идентификатора сессии (session UUID), который автоматически удаляется при закрытии браузера.</p>
...
<p>Цель регистрации — улучшение поисковых результатов и контента сайта. Данные автоматически удаляются через 90 дней. Они не передаются третьим лицам, не используются для рекламы и не связаны с вашей личностью.</p>
""".strip()

# Strapi document ID for the privacy page (shared across locales)
...
def _update_page(locale: str, content: str) -> dict[str, Any]:
    """PUT updated content via Strapi v5 document ID. Strapi auto-publishes."""
    base = _strapi_base_url()
    url = f"{base}/api/pages/{PRIVACY_DOCUMENT_ID}?locale={locale}"
...
def run(target_name: str) -> int:
    ...
    print(f"Adding search privacy disclosure to privacy pages...")
    print()
    ...
    for locale, (title, disclosure) in locales.items():
        ...
        # Check if disclosure is already present (idempotent)
