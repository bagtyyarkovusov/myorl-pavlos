...
1. **Analyze** — [`slug_parity_analyze.py`](../../tools/slug_parity_analyze.py) joins
   [`published_resources_flat.json`](../../data/source/modx/published_resources_flat.json) (or
   [`transformed_resources.json`](../../data/source/modx/transformed_resources.json)),
   [`checkpoint.json`](../../data/source/checkpoints/checkpoint.json) (`pages.web` / `pages.rus` → Strapi
   `documentId`), and live Strapi `GET /api/pages` to emit
   [`slug_parity_report.json`](../../artifacts/reports/slug_parity_report.json). Each row includes
...
3. **Apply** — [`slug_parity_apply.py`](../../tools/slug_parity_apply.py) defaults to
   **dry-run** (logged `PUT`s, no writes). It sends **`strapi_slug_resolved`**
   ...
   [`slug_migration_errors.json`](../../artifacts/reports/slug_migration_errors.json).
...
4. **Dependent content** — After slug changes, re-run
   [`internal_link_rewrite.py`](../../tools/internal_link_rewrite.py) on transformed HTML
   (or a Strapi-side href pass) so `content` / TVs / `pageBlocks` do not keep
   `/{old-slug}` links. Then run
   [`sync_navigation_from_pages.py`](../../tools/sync_navigation_from_pages.py) with
   `--merge` so navigation `path` / `uiRouterKey` match the new slugs (see
   [`docs/admin-hierarchy-ux.md`](../admin-hierarchy-ux.md)).

5. **Redirects for Next.js** — [`emit_slug_redirects.py`](../../tools/emit_slug_redirects.py)
   reads the MODX export + [`slug_parity_report.json`](../../artifacts/reports/slug_parity_report.json)
   and writes [`slug_redirects_next.json`](../../data/manifests/slug_redirects_next.json): primary
