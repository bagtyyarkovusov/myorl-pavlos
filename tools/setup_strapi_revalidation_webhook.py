#!/usr/bin/env python3
"""Upsert or verify the Strapi webhook that calls the Next.js revalidation route."""
...
WEBHOOK_NAME = "Next.js revalidation"
WEBHOOK_EVENTS = [
    "entry.create",
    "entry.update",
    "entry.delete",
...
def verify_sqlite(db_path: Path, payload: dict[str, Any]) -> list[str]:
    ...
    if sorted(json.loads(row[2] or "[]")) != sorted(WEBHOOK_EVENTS):
        failures.append("events do not match expected revalidation events")
    if int(row[3] or 0) != 1:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--url", help="Full webhook URL. Defaults to NEXT_PUBLIC_SITE_URL/api/revalidate.")
    parser.add_argument("--secret", help="Bearer secret. Defaults to STRAPI_REVALIDATE_SECRET.")
...
def main() -> int:
    ...
    args = parse_args()
    secret = args.secret or os.environ.get("STRAPI_REVALIDATE_SECRET")
    if not secret:
        print("[FAIL] STRAPI_REVALIDATE_SECRET is required")
        return 1
    ...
    if args.database_url:
        ...
        print("[PASS] strapi-webhook-upsert: PostgreSQL")
        return 0
    ...
    if failures:
        print("[FAIL] strapi-webhook")
        for failure in failures:
            print(f"  - {failure}")
        return 1

    print(f"[PASS] strapi-webhook: {payload['url']}")
    return 0
