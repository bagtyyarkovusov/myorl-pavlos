...
1.  **`data/source/modx/published_resources.json`**: A nested tree structure (useful for NoSQL backends, headless CMS, or building navigation menus).
2.  **`data/source/modx/published_resources_flat.json`**: A flat array of objects (useful for relational databases).
...
1.  **Hierarchy:** If your target system uses nested relationships, import `data/source/modx/published_resources.json` recursively. If it uses a relational `parent_id` column, import `data/source/modx/published_resources_flat.json` and ensure the parent rows are inserted before child rows to satisfy foreign key constraints.
