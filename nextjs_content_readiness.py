...
REPORT_JSON_PATH = ROOT / "nextjs_content_readiness.json"
REPORT_MD_PATH = ROOT / "docs" / "nextjs-content-readiness.md"
TRANSFORMED_PATH = ROOT / "transformed_resources.json"
CHECKPOINT_PATH = ROOT / "checkpoint.json"
STRUCTURAL_MANIFEST_PATH = ROOT / "nextjs_structural_review_manifest.json"
LEGACY_MANIFEST_PATH = ROOT / "nextjs_legacy_cleanup_manifest.json"
...
ALIGNMENT_MANIFEST_SCRIPT_PATH = ROOT / "build_nextjs_alignment_manifests.py"
ADR_DIR = ROOT / "docs" / "adr"
PAGES_INDEX_SQL_PATH = ROOT / "backend" / "database" / "postgres-readiness" / "001_pages_lookup_indexes.sql"
TAGS_INDEX_SQL_PATH = ROOT / "backend" / "database" / "postgres-readiness" / "002_tag_slug_indexes.sql"
...
def score_operational_readiness(*, query_plans_present_full_scan: bool) -> int:
    score = 10
    if query_plans_present_full_scan:
        score -= 2
    if not PAGES_INDEX_SQL_PATH.exists() or not TAGS_INDEX_SQL_PATH.exists():
        score -= 1
    score -= 1
    score -= 1
    return max(score, 0)
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    structural_manifest = STRUCTURAL_MANIFEST_PATH.name
    legacy_manifest = LEGACY_MANIFEST_PATH.name
    ...
    lines.extend(
        [
            ...
            "## Artifacts",
            "",
            f"- Machine-readable readiness report: [{report_json}](../{report_json})",
